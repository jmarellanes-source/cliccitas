...
class PbxApiService {
  ...
  async getNextBusinessId() {
    // La API solo permite máximo 100 registros por petición
    // Si tienes más de 100 grupos, necesitas paginación
    let allGroups = [];
    let skip = 0;
    const top = 100;
    let hasMore = true;
    ...
    while (hasMore) {
      const groups = await this.request(
        'GET', 
        `/xapi/v1/Groups?$top=${top}&$skip=${skip}`
      );
